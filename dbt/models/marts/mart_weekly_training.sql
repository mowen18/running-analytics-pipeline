...
weekly as (
    ...
    select
        week_start_date,
        count(*)                                          as run_count,
        count(*) filter (where is_qualifying)             as qualifying_run_count,
        ...
        -- as secondary only. Both aggregate QUALIFYING runs only, and
        -- both are NULL — never zero — when no run qualifies.
        percentile_cont(0.5) within group (
            order by aerobic_efficiency_m_per_heartbeat
        ) filter (where is_qualifying)                    as median_efficiency_m_per_beat,
        avg(aerobic_efficiency_m_per_heartbeat)
            filter (where is_qualifying)                  as mean_efficiency_m_per_beat,
        avg(temperature_f) filter (where is_qualifying and weather_available)
                                                          as avg_temperature_f,
        avg(relative_humidity_pct) filter (where is_qualifying and weather_available)
                                                          as avg_relative_humidity_pct,
        count(*) filter (where is_qualifying and weather_available)
                                                          as qualifying_runs_with_weather
    from runs
...
select
    ...
    qualifying_runs_with_weather,
    -- D12: a weekly efficiency point is only trend-worthy with at least
    -- min_weekly_qualifying_runs qualifying easy runs behind it.
    qualifying_run_count >= {{ var('min_weekly_qualifying_runs') }} as is_sufficient
from weekly
